...
class Transform2D:
    ...
    def transform_points(points, M):
        if isinstance(points, Sequence):
            assert len(points) == len(M)
            return [
                Transform2D.transform_points(b, m)
                for b, m in zip(points, M)
            ]
        score = None
        if points.shape[1] > 2:
            score = points[:, 2:]
        ...
        if score is not None:
            return torch.cat([points, score], dim=1)
        return points
...
def _transform_points(points, trans_mat):
