...
class MeanGlobal(Hook):
    ...
    def before_run(self, runner):
        ...
        if is_module_wrapper(model):
            model = model.module
        assert hasattr(model, "globals")
        assert hasattr(model, "locals")
        # only do it at initial stage
        if runner.iter == 0:
            log_every_n("Clone all parameters of locals to globals...")
    ...
    def momentum_update(self, model, momentum):
        for (src_name, src_parm), (tgt_name, tgt_parm) in zip(
            model.locals.named_parameters(), model.globals.named_parameters()
        ):
